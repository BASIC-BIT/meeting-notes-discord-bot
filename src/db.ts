...
import { AccessLog, PaymentTransaction, RecordingTranscript, Subscription } from "./types/db";

const dynamoDbClient = new DynamoDBClient({ region: 'us-east-1' });

// Write to Subscription Table
export async function writeSubscription(subscription: Subscription): Promise<void> {
    const params = {
...
export async function writePaymentTransaction(transaction: PaymentTransaction): Promise<void> {
    const params = {
...
export async function getPaymentTransaction(transactionID: string): Promise<PaymentTransaction | undefined> {
    const params = {
        TableName: 'PaymentTransactionTable',
...
export async function getAccessLog(accessLogID: string): Promise<AccessLog | undefined> {
    const params = {
        TableName: 'AccessLogsTable',
...
export async function writeRecordingTranscript(recordingTranscript: RecordingTranscript): Promise<void> {
    const params = {
...
export async function getRecordingTranscript(meetingID: string): Promise<RecordingTranscript | undefined> {
    const params = {
        TableName: 'RecordingTranscriptTable',
